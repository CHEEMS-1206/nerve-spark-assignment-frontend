...
<script>
	...
	const handleSubmit = async () => {
		...
		try {
			const response = await fetch(`http://localhost:5001/api/${userType.toLowerCase()}/register`, {
				method: 'POST',
				headers: {
...
<main>
	...
	{:else if userType === 'User'}
		<form on:submit={handleSubmit}>
			...
			<input type="text" id="user_location" bind:value={formData.user_location} required />

			<label for="user_password">User Password:</label>
			<input type="password" id="user_password" bind:value={formData.user_password} required />

			<label for="user_info_name">Name:</label>
			<input type="text" id="user_info_name" bind:value={formData.user_info_name} required />

			<label for="user_info_mob_num">Mobile Number:</label>
			<input
				type="number"
				id="user_info_mob_num"
				bind:value={formData.user_info_mob_num}
				pattern="[0-9]{10}"
				required
			/>

			<label for="user_info_age">Age:</label>
			<input type="number" id="user_info_age" bind:value={formData.user_info_age} required />
	...
	{:else if userType === 'Dealership'}
		<form on:submit={handleSubmit}>
			...
			<input
				...
				bind:value={formData.dealership_location}
				required
			/>
			...
			<input type="text" id="dealership_name" bind:value={formData.dealership_name} required />

			<label for="dealership_info_contact_num">Contact Number:</label>
			<input
				type="number"
				id="dealership_info_contact_num"
				bind:value={formData.dealership_info_contact_num}
				pattern="[0-9]{10}"
				required
			/>

			<label for="dealership_info_rating">Rating:</label>
			<input
				type="number"
				id="dealership_info_rating"
				bind:value={formData.dealership_info_rating}
				required
			/>
